<?php 
	...
	function qltv_xoa_ls($ma){
		...
		if (qltv_kiem_tra_ton_tai("SELECT MaL FROM lop l, khoa k WHERE l.MaK = k.MaK AND k.MaK = '$ma'")) {
			echo "<script type=\"text/javascript\">khongthanhcong(\"<strong>Chưa xóa</strong> không thể xóa bưu cục khi khi còn chứa các lớp!\")</script>";
			exit();
		}
		$hoi = "
	...
	if (isset($_SESSION['username']) && isset($_SESSION['password'])) {
		if(!qltv_login_tt($_SESSION['username'],$_SESSION['password'])){
			...
			if (qltv_xoa_ls($_POST['ma'])) {
				echo "<script type=\"text/javascript\">tailai();thanhcong(\"<strong>Đã xóa</strong> bưu cục ".$_POST['ma']."!\")</script>";
				exit();
			}
			else{
