<?php 
	...
	function qltv_xoa_ls($ma){
		...
		if ($dem_kiemtra > 0){
			...
			if($mang_kiemtra[0] > 0){
				echo "<script>khongthanhcong(\"Bạn không thể xóa xuất xứ này. Đang có ".$mang_kiemtra[0]." thiết bị thuộc xuất xứ này trong kho!\")</script>";
				exit();
			}
		}
	...
	if (isset($_SESSION['username']) && isset($_SESSION['password'])) {
		if(!qltv_login_tt($_SESSION['username'],$_SESSION['password'])){
			...
			if (qltv_xoa_ls($_POST['ma'])) {
				echo "<script type=\"text/javascript\">tailai();thanhcong(\"<strong>Đã xóa</strong> xuất xứ!\")</script>";
				exit();
			}
			else{
