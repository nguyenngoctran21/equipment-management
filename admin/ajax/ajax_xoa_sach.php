<?php 
	...
	function qltv_xoa_sach($ma){
		...
		if (qltv_kiem_tra_ton_tai("SELECT mt.Id FROM muontra mt, sach s WHERE s.MaS = mt.MaS AND s.MaS = '$ma' AND mt.TrangThai = '0'")) {
			echo "<script type=\"text/javascript\">khongthanhcong(\"<strong>Chưa xóa</strong> không thể xóa thiết bị này, thiết bị này hiện đang được mượn (chưa thu hồi hết)!\")</script>";
			exit();
		}
	...
	if (isset($_SESSION['username']) && isset($_SESSION['password'])) {
		if(!qltv_login_tt($_SESSION['username'],$_SESSION['password'])){
			...
			if (qltv_xoa_sach($_POST['ma'])) {
				echo "<script type=\"text/javascript\">tailai();thanhcong(\"<strong>Đã xóa</strong> thiết bị!\")</script>";
				exit();
			}
			else{
