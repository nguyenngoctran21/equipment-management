<?php 
	...
	function qltv_xoa_ls($ma){
		...
		$kiemtra = "Select count(MaPX) as so from muontra where MaPX = '$ma'";
		$kiemtra_ec = mysqli_query($conn, $kiemtra);
		$dem_kiemtra = mysqli_num_rows($kiemtra_ec);
		if ($dem_kiemtra > 0){
			$mang_kiemtra = mysqli_fetch_array($kiemtra_ec);
			if($mang_kiemtra[0] > 0){
				echo "<script>khongthanhcong(\"Bạn không thể xóa phiếu xuất này. Đang có ".$mang_kiemtra[0]." thiết bị thuộc phiếu xuất này trong kho!\")</script>";
				exit();
			}
		}
	...
	if (isset($_SESSION['username']) && isset($_SESSION['password'])) {
		if(!qltv_login_tt($_SESSION['username'],$_SESSION['password'])){
			...
			if (qltv_xoa_ls($_POST['ma'])) {
				echo "<script type=\"text/javascript\">tailai();thanhcong(\"<strong>Đã xóa</strong> phiếu xuất!\")</script>";
				exit();
			}
			else{
