<?php 
	...
	function qltv_xoa_lop($ma){
		...
		if (qltv_kiem_tra_ton_tai("SELECT dg.MaL FROM lop l, docgia dg WHERE dg.MaL = l.MaL AND l.MaL = '$ma'")) {
			echo "<script type=\"text/javascript\">khongthanhcong(\"<strong>Chưa xóa</strong> không thể xóa phòng ban khi khi còn chứa người dùng!\")</script>";
			exit();
		}
		$hoi = "
	...
	if (isset($_SESSION['username']) && isset($_SESSION['password'])) {
		if(!qltv_login_tt($_SESSION['username'],$_SESSION['password'])){
			...
			if (qltv_xoa_lop($_POST['ma'])) {
				echo "<script type=\"text/javascript\">tailai();thanhcong(\"<strong>Đã xóa</strong> phòng ban ".$_POST['ma']."!\")</script>";
				exit();
			}
			else{
